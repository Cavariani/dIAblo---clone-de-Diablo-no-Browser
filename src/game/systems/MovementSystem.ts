...
export class MovementSystem implements System {
  ...
  update(ctx: GameCtx, dt: number): void {
    ...
    for (const a of w.actors) {
      ...
      if (moving && (a.vel.x !== 0 || a.vel.y !== 0)) a.facing = Math.atan2(a.vel.y, a.vel.x);
      // locomotion animation
      if (!a.cast && (a.state === 'idle' || a.state === 'moving')) {
        const walking = a.vel.x * a.vel.x + a.vel.y * a.vel.y > 0.01;
        a.state = walking ? 'moving' : 'idle';
        const runSpeed = a.moveSpeed / Math.max(0.5, a.kind === 'player' ? 2.6 : a.baseMoveSpeed || 1);
